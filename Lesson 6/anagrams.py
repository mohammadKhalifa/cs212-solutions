...
def anagrams(phrase, shortest=2):
    ...
    # your code here
    results = set()
    letters = ''.join(phrase.split())
    anagrams_aux('',letters , results , shortest)
    ret = set()
    for res in results:
        ret.add(' '.join(sorted(res.split())))
    return ret
...
def find_words(letters):
    return anagrams_aux('', letters, set())

def anagrams_aux(pre, letters, results , shortest = 2): 
    if pre and pre.split()[-1] in WORDS and not letters:
        results.add(pre)
        return  
    if pre and pre.split()[-1] in WORDS and len(pre.split()[-1]) >=shortest :
        for L in letters:
            anagrams_aux(pre+' '+L , letters.replace(L,'',1) , results , shortest) 
    if not pre or pre.split()[-1] in PREFIXES:
        for L in letters:
            anagrams_aux(pre+L , letters.replace(L,'',1) , results ,shortest)
    return results
